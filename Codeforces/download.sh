...
if [[ "$FILE_EXT" == "html" ]]; then
    # CSS dosyalarının konumu
    CSS_DIR="/Applications/XAMPP/xamppfiles/htdocs/Codeforces/css/"
    
    # CSS dosyalarını listele
    CSS_FILES=$(ls "$CSS_DIR"*.css 2>/dev/null)
    
    if [ ! -z "$CSS_FILES" ]; then
        # CSS dosyalarını HTML dosyasının sonuna ekle
        for CSS_FILE in $CSS_FILES; do
            CSS_NAME=$(basename "$CSS_FILE")
            echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"/Codeforces/css/$CSS_NAME\">" >> "$LOCAL_FILE"
        done
    fi
fi